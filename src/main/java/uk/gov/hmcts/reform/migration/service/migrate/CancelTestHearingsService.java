...
public class CancelTestHearingsService extends CaseMigrationProcessor {
    ...
    public CancelTestHearingsService(ElasticSearchRepository repository, CancelTestHearingsSearchQuery searchQuery, HmcHearingsApiService hmcHearingsApiService) {
    ...
    public UpdateResult migrate(CaseDetails caseDetails) {
        HearingsGetResponse hmcResponse = hmcHearingsApiService.getHearingsRequest(caseDetails.getId().toString(), HmcStatus.AWAITING_LISTING);
        if (hmcResponse.getCaseHearings() != null && !hmcResponse.getCaseHearings().isEmpty()) {
            for (CaseHearing caseHearing : hmcResponse.getCaseHearings()) {
                HearingCancelRequestPayload cancelRequestPayload = HearingCancelRequestPayload.builder().cancellationReasonCodes(
                    Collections.singletonList(CancellationReason.OTHER)).build();
                HearingsUpdateResponse response = hmcHearingsApiService.sendCancelHearingRequest(cancelRequestPayload, caseHearing.getHearingId().toString());
                log.debug("Received Cancel Hearing Request Response for Case ID {}:\n{}",
                          caseDetails.getId(),
                          response);
            }
        } else {
            log.info("Skipping case for cancel hearings job, no hearings awaiting listing found for CaseId {}", caseDetails.getId());
